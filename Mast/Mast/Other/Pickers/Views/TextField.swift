...
open class TextField1: UITextField {
    
    public typealias Config = (TextField1) -> Swift.Void
    ...
    public func action(closure: @escaping Action) {
        if actionEditingChanged == nil {
            addTarget(self, action: #selector(TextField1.textFieldDidChange), for: .editingChanged)
        }
