...
class FollowRequestsViewController: UIViewController, UITextFieldDelegate, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = GlobalStruct.baseDarkTint
        self.title = "Follow Requests".localized
//        self.removeTabbarItemsText()
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return self.statusesRequests.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "FollowersCell", for: indexPath) as! FollowersCell
        if self.statusesRequests.isEmpty {} else {
            cell.configure(self.statusesRequests[indexPath.row])
    ...
    @objc func viewProfile(_ gesture: UIGestureRecognizer) {
        let vc = FifthViewController()
        if GlobalStruct.currentUser.id == (self.statusesRequests[gesture.view!.tag].id) {
            vc.isYou = true
        } else {
            vc.isYou = false
        }
        vc.pickedCurrentUser = self.statusesRequests[gesture.view!.tag]
        self.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let vc = FifthViewController()
        vc.isYou = false
        vc.pickedCurrentUser = self.statusesRequests[indexPath.row]
        self.navigationController?.pushViewController(vc, animated: true)
    }
    ...
    func tableView(_ tableView: UITableView,
        ...
        return UIContextMenuConfiguration(identifier: indexPath as NSIndexPath, previewProvider: { return nil }, actionProvider: { suggestedActions in
            return self.makeContextMenu([self.statusesRequests[indexPath.row]], indexPath: indexPath)
        })
    }
    
    func makeContextMenu(_ status: [Account], indexPath: IndexPath) -> UIMenu {
        let approve = UIAction(title: "Approve".localized, image: UIImage(systemName: "person.crop.circle.badge.checkmark"), identifier: nil) { action in
            let request = FollowRequests.authorize(id: self.statusesRequests[indexPath.row].id)
            GlobalStruct.client.run(request) { (statuses) in
                DispatchQueue.main.async {
                    self.statusesRequests.remove(at: indexPath.row)
                    self.tableView.reloadData()
                }
            }
        }
        let remove = UIAction(title: "Reject".localized, image: UIImage(systemName: "person.crop.circle.badge.xmark"), identifier: nil) { action in
            let request = FollowRequests.reject(id: self.statusesRequests[indexPath.row].id)
            GlobalStruct.client.run(request) { (statuses) in
                DispatchQueue.main.async {
                    self.statusesRequests.remove(at: indexPath.row)
                    self.tableView.reloadData()
