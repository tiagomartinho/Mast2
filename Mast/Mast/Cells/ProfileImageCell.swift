...
class ProfileImageCell: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if self.profileStatusesImages[indexPath.item].mediaAttachments[0].type == .video {
            ...
            let imageInfo = GSImageInfo(image: self.images2[indexPath.item].image ?? UIImage(), imageMode: .aspectFit, imageHD: URL(string: self.images3[indexPath.item]), imageText: "@\(self.currentStat[indexPath.item].account.username): \(self.currentStat[indexPath.item].content.stripHTML())", imageText2: self.currentStat[indexPath.item].favouritesCount, imageText3: self.currentStat[indexPath.item].reblogsCount)
