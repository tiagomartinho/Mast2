...
class DetailActionsCell: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        button1.setImage(UIImage(systemName: "arrowshape.turn.up.left", withConfiguration: symbolConfig)?.withTintColor(UIColor(named: "baseBlack")!.withAlphaComponent(0.5), renderingMode: .alwaysOriginal), for: .normal)
        ...
        button4.setImage(UIImage(systemName: "square.and.arrow.up", withConfiguration: symbolConfig)?.withTintColor(UIColor(named: "baseBlack")!.withAlphaComponent(0.5), renderingMode: .alwaysOriginal), for: .normal)
        containerView.addSubview(button4)
        
        button5.translatesAutoresizingMaskIntoConstraints = false
        button5.backgroundColor = UIColor.clear
        button5.setImage(UIImage(systemName: "ellipsis.circle", withConfiguration: symbolConfig)?.withTintColor(UIColor(named: "baseBlack")!.withAlphaComponent(0.5), renderingMode: .alwaysOriginal), for: .normal)
    ...
    func configure(_ stat: Status) {
        let symbolConfig = UIImage.SymbolConfiguration(pointSize: 21, weight: .regular)
        if stat.favourited ?? false || GlobalStruct.allLikedStatuses.contains(stat.reblog?.id ?? stat.id) {
